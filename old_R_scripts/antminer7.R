...
antminer7 <- function(trainingSet,class, maxUncoveredCases, NumberOfAnts, NumberOfRulesConverge, MinCasesPerRule) {
  ...
  while(nrow(trainingSet) > maxUncoveredCases) {
    ...
    #print(nrow(trainingSet))
    #numer mrowki#pierwsze kryterium - najdluzsze reguly
    #Wybieramy reguly najbardziej pasujace
    #pierwsze kryterium - najdluzsze reguly
    #Wybieramy reguly najbardziej pasujace    #indeks testu zbieznosci
    j <- 1;
    ...
    pheromones <- lapply(terms, function(x) {sapply(x, function(y) {namedPheromone7(y,initialPheromone)})})
    ...
    while( (i<NumberOfAnts) & (j<NumberOfRulesConverge) ) {
      ...
      while(sum.used_attributes > 0 & isMinCasesPerRule) {
        #######compute eta####################################
        etas <- lapply(entropies ,function(x) {sapply(x, function(entropy) {eta7(nr_of_class, x, entropy, rule.used_attributes)})})
        #######koniec compute eta
        ...
        unlistTerms <- unlist(removeUsedTerms7(terms, rule.used_attributes))
        size <- length(unlistTerms)
        drawnTerms <- sample(unlistTerms, size, FALSE, unlist(removeUsedTerms7(probabilities, rule.used_attributes)))
        isMinCasesPerRule<-FALSE
        for(q in 1:size) {
          #wybierz z wylosowanych term'ow term ktory nie nalezy do
          #atrybutow juz wybranych
          attribute_id <- getAttributeId7(terms, drawnTerms[q])
      ...
      if(addedTermIndex == 1) {
        ...
        rule[[1]][addedTermIndex]<-majorClass27(trainingSet2, class)
      ...
      rule[[3]] <- quality7(rule, trainingSet, class, columnNames)
      n<-length(rule[[2]])
      while( n > 2 & isBetterQuality) {
        qualities<-sapply(1:(n-1), function(k) {quality7(removeTerm7(rule,k), trainingSet, class, columnNames)})
      ...
      rule[[1]][n]<-majorClass7(coveredCases7(rule[[1]], rule[[2]], trainingSet, columnNames), class, trainingSet)
    ...
    if(length(rules)>0) {
      ...
      a<-coveredCases7(bestRule[[1]], bestRule[[2]], trainingSet, columnNames)
  ...
  defaultClass <- majorClass27(trainingSet, class)
  model <- list(discoveredRules, defaultClass)
  class(model)<-"antminer7"
  return(model)
}
...
getTerms7<-function(trainingSet) {
  ...
  lapply(1:length(terms), function(x) {lapply(terms[[x]], function(term) {namedTerm7(term, terms[x])})})
}

namedTerm7<-function(term, terms) {
...
computeEntropy7 <- function(terms, data, class) {
  mapply(function(terms, index) {sapply(terms, function(x) {entropy7(x, index, data, class)})}, terms, 1:length(terms), SIMPLIFY = FALSE)
}

#oblicza entropie danego terma, czyli entropie pary atrybut-wartosc
entropy7 <- function(term, col, data, class) {
...
initPheromone <- function(terms) {
  ...
  lapply(terms, function(x) {sapply(x, function(y) {namedPheromone7(y,initialPheromone)})})

  #lapply(1:length(terms), function(i) {lapply(terms[[i]], function(y) {initialPheromone})})
}

namedPheromone7<-function(term, pheromone) {
  names(pheromone)<-term
  return (pheromone)
}

removeUsedTerms7<-function(terms, used_attributes) {
...
eta7 <- function(nr_of_class, entropies, entropy, used_attributes) {
...
getAttributeId7 <- function(terms, term) {
  #min(which(sapply(terms, function(x) {is.element(term, x)}) == TRUE))
  which(sapply(sapply(terms, function(x) {sapply(x, function(y) {checkEqualTerm7(term, y)})}), function(row) {is.element(TRUE, row)}) == TRUE)
}

#sprawdza czy termy ma taka sama wartosc i nazwe
checkEqualTerm7<-function(term1, term2) {
  term1 == term2 & names(term1) == names(term2)
}

#pokryte przez regule przypadki
#zwraca liste wierszy z pokrytymi przypadkami
coveredCases7 <- function(rule, rule.attributes, trainingSet, columnNames) {
...
majorClass7 <- function(coveredCases, class, trainingSet) {
...
majorClass27<-function(trainingSet, class) {
  names(which.max(table(trainingSet[,class, with=FALSE])))
}

removeTerm7<-function(rule, index) {
  rule[[1]]<-rule[[1]][-index]
  rule[[2]]<-rule[[2]][-index]
  rule
}
...
quality7 <- function(rule, trainingSet, class, columnNames) {
  n<-length(rule[[1]])
...
isEqualRule7 <- function(rule1, rules) {
  if(length(rules)==0) {
    return (FALSE)
  }
...
predict.antminer7 <- function(model, data) {
  discoveredRules <- model[[1]]
  defaultClass <- model[[2]]
  test<-apply(data,1, function(x) {
    result <- sapply(discoveredRules, function(y) {isCoveredByRule7(y,x)})
...
isCoveredByRule7 <- function(rule, case) {
  al <- rule[[2]]
  rule <- rule[[1]]
  n<-length(rule)
  for(i in 1:(n-1)) {

...
getAttributeId2 <- function(terms, term) {
  ...
  which(sapply(sapply(terms, function(x) {sapply(x, function(y) {checkEqualTerm2(term, y)}, simplify = FALSE)}, simplify = FALSE), function(row) {is.element(TRUE, row)}, simplify = FALSE) == TRUE)
}

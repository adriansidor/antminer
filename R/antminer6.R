...
antminer6 <- function(trainingSet,class, maxUncoveredCases, NumberOfAnts, NumberOfRulesConverge, MinCasesPerRule) {
  ...
  while(nrow(trainingSet) > maxUncoveredCases) {
    ...
    pheromones <- lapply(terms, function(x) {sapply(x, function(y) {namedPheromone6(y,initialPheromone)})})
    ...
    while( (i<NumberOfAnts) & (j<NumberOfRulesConverge) ) {
      ...
      while(sum.used_attributes > 0 & isMinCasesPerRule) {
        #######compute eta####################################
        etas <- lapply(entropies ,function(x) {sapply(x, function(entropy) {eta6(nr_of_class, x, entropy, rule.used_attributes)})})
        #######koniec compute eta
        ...
        unlistTerms <- unlist(removeUsedTerms6(terms, rule.used_attributes))
        size <- length(unlistTerms)
        drawnTerms <- sample(unlistTerms, size, FALSE, unlist(removeUsedTerms6(probabilities, rule.used_attributes)))
        isMinCasesPerRule<-FALSE
        for(q in 1:size) {
          #wybierz z wylosowanych term'ow term ktory nie nalezy do
          #atrybutow juz wybranych
          attribute_id <- getAttributeId6(terms, drawnTerms[q])
      ...
      if(addedTermIndex == 1) {
        ...
        rule[[1]][addedTermIndex]<-majorClass26(trainingSet2, class)
      ...
      rule[[3]] <- quality6(rule, trainingSet, class, columnNames)
      n<-length(rule[[2]])
      while( n > 2 & isBetterQuality) {
        qualities<-sapply(1:(n-1), function(k) {quality6(removeTerm6(rule,k), trainingSet, class, columnNames)})
      ...
      rule[[1]][n]<-majorClass6(coveredCases6(rule[[1]], rule[[2]], trainingSet, columnNames), class, trainingSet)
      ...
      for(q in 1:(n-1)) {
        ...
        pheromones[[attributeId]][termid]<-pheromones[[attributeId]][termid] + (pheromones[[attributeId]][termid]*rule[[3]])
      }
    ...
    if(length(rules)>0) {
      ...
      trainingSet<-trainingSet[-coveredCases6(bestRule[[1]], bestRule[[2]], trainingSet, columnNames)]
    } else {
      print("przerywam algorytm")
      break;
    }
  }
  defaultClass <- majorClass26(trainingSet, class)
  model <- list(discoveredRules, defaultClass)
  class(model)<-"antminer6"
  return(model)
}
...
getTerms6<-function(trainingSet) {
  ...
  lapply(1:length(terms), function(x) {lapply(terms[[x]], function(term) {namedTerm6(term, terms[x])})})
}

namedTerm6<-function(term, terms) {
...
computeEntropy6 <- function(terms, data, class) {
  mapply(function(terms, index) {sapply(terms, function(x) {entropy6(x, index, data, class)})}, terms, 1:length(terms))
}

#oblicza entropie danego terma, czyli entropie pary atrybut-wartosc
entropy6 <- function(term, col, data, class) {
...
initPheromone <- function(terms) {
  ...
  lapply(terms, function(x) {sapply(x, function(y) {namedPheromone6(y,initialPheromone)})})

  #lapply(1:length(terms), function(i) {lapply(terms[[i]], function(y) {initialPheromone})})
}

namedPheromone6<-function(term, pheromone) {
  names(pheromone)<-term
  return (pheromone)
}

removeUsedTerms6<-function(terms, used_attributes) {
...
eta6 <- function(nr_of_class, entropies, entropy, used_attributes) {
...
getAttributeId6 <- function(terms, term) {
  #min(which(sapply(terms, function(x) {is.element(term, x)}) == TRUE))
  which(sapply(sapply(terms, function(x) {sapply(x, function(y) {checkEqualTerm6(term, y)})}), function(row) {is.element(TRUE, row)}) == TRUE)
}

#sprawdza czy termy ma taka sama wartosc i nazwe
checkEqualTerm6<-function(term1, term2) {
  term1 == term2 & names(term1) == names(term2)
}

#pokryte przez regule przypadki
#zwraca liste wierszy z pokrytymi przypadkami
coveredCases6 <- function(rule, rule.attributes, trainingSet, columnNames) {
...
majorClass6 <- function(coveredCases, class, trainingSet) {
...
majorClass26<-function(trainingSet, class) {
  names(which.max(table(trainingSet[,class, with=FALSE])))
}

removeTerm6<-function(rule, index) {
  rule[[1]]<-rule[[1]][-index]
  rule[[2]]<-rule[[2]][-index]
  rule
}
...
quality6 <- function(rule, trainingSet, class, columnNames) {
  n<-length(rule[[1]])
...
isEqualRule6 <- function(rule1, rules) {
  if(length(rules)==0) {
    return (FALSE)
  }
...
predict.antminer6 <- function(model, data) {
  discoveredRules <- model[[1]]
  defaultClass <- model[[2]]
  test<-apply(data,1, function(x) {
    result <- sapply(discoveredRules, function(y) {isCoveredByRule6(y,x)})
...
isCoveredByRule6 <- function(rule, case) {
  al <- rule[[2]]
  rule <- rule[[1]]
  n<-length(rule)
  for(i in 1:(n-1)) {

  ...
  while(nrow(trainingSet) > maxUncoveredCases) {
    ...
    while( (i<NumberOfAnts) & (j<NumberOfRulesConverge) ) {
      ...
      while( n > 2 & isBetterQuality) {
        qualities<-sapply(1:(n-1), function(k) {quality6(removeTerm(rule,k), trainingSet, class, columnNames)})
...
prune <- function(rule, trainingSet, class) {
  isBetterQuality <- TRUE;
  bestQuality <- quality6(rule, trainingSet, class)
  n<-length(rule[[2]])
  while( n > 2 & isBetterQuality) {
    qualities<-sapply(1:(n-1), function(i) {quality6(removeTerm(rule,i), trainingSet, class)})
    id<- which.max(qualities)
    maxquality <- qualities[id]

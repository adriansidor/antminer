...
antminer3 <- function(trainingSet,class, maxUncoveredCases, NumberOfAnts, NumberOfRulesConverge, MinCasesPerRule) {
  ...
  entropies<-computeEntropy3(terms, trainingSet, class)
  ...
  while(nrow(trainingSet) > maxUncoveredCases) {
    ...
    pheromones <- lapply(terms, function(x) {sapply(x, function(y) {namedPheromone3(y,initialPheromone)})})
    ...
    while( (i<NumberOfAnts) & (j<NumberOfRulesConverge) ) {
      ...
      while(sum.used_attributes > 0 & isMinCasesPerRule) {
        #######compute eta####################################
        etas <- lapply(entropies ,function(x) {sapply(x, function(entropy) {eta3(nr_of_class, x, entropy, rule.used_attributes)})})
        #######koniec compute eta
        ...
        drawnTerms <- sample(unlistTerms, size, FALSE, unlist(removeUsedTerms3(probabilities, rule.used_attributes)))
        isMinCasesPerRule<-FALSE
        for(q in 1:size) {
          #wybierz z wylosowanych term'ow term ktory nie nalezy do
          #atrybutow juz wybranych
          attribute_id <- getAttributeId3(terms, drawnTerms[q])
      ...
      if(addedTermIndex == 1) {
        ...
        rule[[1]][addedTermIndex]<-majorClassb3(trainingSet2, class)
      ...
      rule[[3]] <- quality3(rule, trainingSet, class, columnNames)
      n<-length(rule[[2]])
      while( n > 2 & isBetterQuality) {
        qualities<-sapply(1:(n-1), function(k) {quality3(removeTerm3(rule,k), trainingSet, class, columnNames)})
      ...
      rule[[1]][n]<-majorClass3(coveredCases3(rule[[1]], rule[[2]], trainingSet, columnNames), class, trainingSet)
  ...
  defaultClass <- majorClassb3(trainingSet, class)
  model <- list(discoveredRules, defaultClass)
  class(model)<-"antminer3"
  return(model)
}
...
computeEntropy3 <- function(terms, data, class) {
  mapply(function(terms, index) {sapply(terms, function(x) {entropy3(x, index, data, class)})}, terms, 1:length(terms), SIMPLIFY = FALSE)
}
...
removeUsedTerms3<-function(terms, used_attributes) {
...
eta3 <- function(nr_of_class, entropies, entropy, used_attributes) {
...
majorClass3 <- function(coveredCases, class, trainingSet) {
...
majorClassb3<-function(trainingSet, class) {
  names(which.max(table(trainingSet[,class, with=FALSE])))
}

removeTerm3<-function(rule, index) {
  rule[[1]]<-rule[[1]][-index]
  rule[[2]]<-rule[[2]][-index]
  rule
}

getFilter3<-function(rule) {
  mapply(function(x,y) {})
}

#mierzy jakosc reguly
quality3 <- function(rule, trainingSet, class, columnNames) {
  n<-length(rule[[1]])
...
isEqualRule3 <- function(rule1, rules) {
  if(length(rules)==0) {
    return (FALSE)
  }
...
isCoveredByRule3 <- function(rule, case) {
  al <- rule[[2]]
  rule <- rule[[1]]
  n<-length(rule)
  for(i in 1:(n-1)) {

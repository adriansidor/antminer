...
for(t in 1:test) {
  folds <- createFolds(breast_cancer_wisconsin$evaluation, k = loop_size, returnTrain = TRUE)
...
for(t in 1:test) {
  folds <- createFolds(nursery$evaluation, k = loop_size, returnTrain = TRUE)
...
for(t in 1:test) {
  folds <- createFolds(mushroom$evaluation, k = loop_size, returnTrain = TRUE)
